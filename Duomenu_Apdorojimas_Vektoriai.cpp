...
double Mediana(vector<Universitetas> A, int e, int i)
{
	...
	{
		A[i].Galutinis = ((double)A[i].Pazymiai[e / 2] + A[i].Pazymiai[(e / 2) - 1]) / 2;
	}
	else
	{
		A[i].Galutinis = A[i].Pazymiai[e / 2];
	}

	return A[i].Galutinis;
}
...
{
	for (int x = 0; x < e; x++)
		A[i].Galutinis += A[i].Pazymiai[x];

	A[i].Galutinis = A[i].Galutinis / e;
