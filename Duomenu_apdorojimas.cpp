...
void Ilgiausias(Universitetas* A, int n, int &Vardas, int &Pavarde) // Funkcija ilgiausiam vardui ar pavardei rasti
{
	int MAX_V = 0;
	int MAX_PV = 0;
	int ilgis1 = 0;
...
int main()
{
	...
	Universitetas* Studentas = new Universitetas[n];
	...
	for (int i = 0; i < n; i++)
	{
		...
		std::sort((Studentas + i)->Pazymiai, (Studentas + i)->Pazymiai + e); // Surikiuojam pazymius
		...
		if (Med_Vid)// Mediana
		{
			...
			{
				(Studentas + i)->Galutinis = Studentas->Pazymiai[e / 2];
			}
		}
		else //Vidurkis
		{
	...
	int Ilgis = Ilgiausias_Vardas + Ilgiausia_Pavarde + 19;
